      ...
      <div class="login-content">
        <form>
          ...
          <div :class="{current: !loginMode}">
            <section>
              <section class="login-message">
                <input v-model="userName" type="tel" maxlength="11" placeholder="用户名" />
              </section>
              <section class="login-verification">
                <input
                  v-if="pwd"
...
<script>
import { getPhoneCode, phoneCodeLogin, pwdLogin } from "../../server/api/index";
  ...
  methods: {
    ...
    async login() {
      ...
      if (this.loginMode) {
        ...
        // 3.2.1 发起请求
        let result = await pwdLogin(this.userName, this.password, this.captcha);
        console.log(result);
        if (result.success_code === 200) {
          // 4.1 保存用户信息
          this.syncUserInfo(result.data);
